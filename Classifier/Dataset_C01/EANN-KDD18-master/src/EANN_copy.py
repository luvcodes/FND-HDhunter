...
class Rumor_Data(Dataset):
    def __init__(self, dataset):
        ...
        # self.text = torch.LongTensor(np.array(dataset['post_text']))
...
class ReverseLayerF(Function):
    ...
    def forward(self, x):
        ...
        ctx = self.save_for_backward(x)  # 保存输入x，以便在backward中访问
        return x.view_as(x)
    ...
    def backward(ctx, grad_output):
        x, = ctx.saved_tensors  # 从ctx中获取保存的输入x
        lambd = args.lambd  # 直接使用类属性self.lambd或全局变量args.lambd
...
def export_to_csv(dataset, filename, columns=['post_text', 'image', 'mask', 'label', 'event_label']):
    with open(filename, 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=columns)
        writer.writeheader()

        for i in range(len(dataset['post_text'])):
            row = {
                'post_text': dataset['post_text'][i],
                'image': dataset['image'][i],
                'mask': dataset['mask'][i],
                'label': dataset['label'][i],
                'event_label': dataset['event_label'][i],
            }
            writer.writerow(row)


def save_datasets_as_pickle(train, validate, test, output_folder):
    """
    Saves train, validate, and test datasets as pickle files.

    Args:
        train (dict): Preprocessed training dataset.
        validate (dict): Preprocessed validation dataset.
        test (dict): Preprocessed test dataset.
        output_folder (str): Path to the folder where the pickle files will be saved.
    """
    # Ensure the output folder exists
    os.makedirs(output_folder, exist_ok=True)

    # Save train dataset
    train_path = os.path.join(output_folder, 'train.pkl')
    with open(train_path, 'wb') as f:
        pickle.dump(train, f)

    # Save validate dataset
    validate_path = os.path.join(output_folder, 'validate.pkl')
    with open(validate_path, 'wb') as f:
        pickle.dump(validate, f)

    # Save test dataset
    test_path = os.path.join(output_folder, 'test.pkl')
    with open(test_path, 'wb') as f:
        pickle.dump(test, f)

    print("Datasets saved as pickle files in:", output_folder)
...
def main(args):
    ...
    # train, validation, test = load_datasets_from_pickle('../Data/weibo/datasets_pickle')



    # 现在encoded_texts是一个二维列表，其中每个元素都是词索引列表
    # 你可以将其转换为PyTorch张量并使用适当的填充或截断策略调整长度
    # W = load_data(args)[3]
    train, validation, test, W = load_data(args)

    # Export train, validation, and test sets
    # Assuming you have preprocessed `train`, `validate`, and `test` datasets
    # save_datasets_as_pickle(train, validation, test, '../Data/weibo/datasets_pickle')
    # export_to_csv(train, 'train.csv')
    # export_to_csv(validation, 'validation.csv')
    # export_to_csv(test, 'test.csv')
    ...
    #
    # # 定义早停参数
    # patience = 10  # 连续多少个epoch验证性能无提升后停止训练
    # min_delta = 1e-4  # 验证性能提升的最小阈值
    ...
    #
    #     # early stop
    #     if validate_acc > best_validate_acc + min_delta:
    ...
    #
    #
    best_validate_dir = '../data/weibo1/RESULT/21.pkl'
...
def parse_arguments(parser):
    ...
    parser.add_argument('--num_epochs', type=int, default=15, help='')
...
def get_top_post(output, label, test_id, top_n = 500):
    ...
    pickle.dump(top_n_id_dict, open("../Data/weibo1/top_n_id.pickle", "wb"))
...
def load_data(args):
    ...
    #print(train[4][0])
    word_vector_path = '../Data/weibo1/word_embedding.pickle'
    f = open(word_vector_path, 'rb')
    weight = pickle.load(f, encoding = 'latin1')  # W, W2, word_idx_map, vocab

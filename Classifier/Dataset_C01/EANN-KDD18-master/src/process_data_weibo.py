...
def stopwordslist(filepath = '../Data/weibo1/stop_words.txt'):
    stopwords = {}
    with open(filepath, 'r', encoding='utf-8') as file:
        for line in file.readlines():
            line = line.strip()  # 不需要显式转换为 Unicode
            stopwords[line] = 1
...
def write_txt(data):
    f = open("../Data/weibo1/top_n_data.txt", 'wb')
    for line in data:
        for l in line:
            f.write(l+"\n")
        f.write("\n")
    ...
    f.close()
text_dict = {}
# 输入处理后的图像列表，输出data（字典形式的post） text_only：是否只处理文本，默认为false
def write_data(flag, image, text_only):
    # 输入txt文件，pickle文件，输出文本内容，post(txt的df表格形式（文本信息）)
    def read_post(flag):
        stop_words = stopwordslist()
        pre_path = "../Data/weibo1/tweets/"
        file_list = [pre_path + "test_nonrumor.txt", pre_path + "test_rumor.txt", \
                         pre_path + "train_nonrumor.txt", pre_path + "train_rumor.txt"]

        # 加载id数据？？？
        if flag == "train":
            id = pickle.load(open("../Data/weibo1/train_id.pickle", 'rb'))
        elif flag == "validate":
            id = pickle.load(open("../Data/weibo1/validate_id.pickle", 'rb'))
        elif flag == "test":
            id = pickle.load(open("../Data/weibo1/test_id.pickle", 'rb'))
...
#         return select(data, select_indices)
    ...
    # post： column = ['post_id', 'image_id', 'original_post', 'post_text', 'label', 'event_label']
    ...
    # 输入post（df），输出data（字典形式的post） text_only：是否只处理文本，默认为false
...
def add_unknown_words(word_vecs, vocab, min_df=1, k=32):
    ...
    word_vecs_dict = {}  # 初始化一个空字典

    for word in vocab:
        if word not in word_vecs and vocab[word] >= min_df:
            word_vecs_dict[word] = np.random.uniform(-0.25, 0.25, k)  # 将词汇和词向量存储到字典中

    # 将字典中的词向量添加到原始的 word_vecs 列表中
    for word, vec in word_vecs_dict.items():
...
def get_data(text_only):
    ...
    #
    #
    word_embedding_path = "../Data/weibo1/w2v.pickle"
    ...
    W2 = rand_vecs = {}
    w_file = open("../Data/weibo1/word_embedding.pickle", "wb")
...
if __name__ == "__main__":
    # image_list：处理过的图像数据列表
    image_list = read_image()
    # 输入处理后的图像列表，输出data（字典形式的post） text_only：是否只处理文本，默认为false
    train_data = write_data("train", image_list, text_only = False)
    valiate_data = write_data("validate", image_list, text_only = False)
    test_data = write_data("test", image_list, text_only = False)

    print("loading data...")
    # w2v_file = '../Data/GoogleNews-vectors-negative300.bin'

    # 输入train和test（数据集的字典形式（paired_data））
    # 输出vocal字典：键是单词，值是单词在所有文本中的总出现次数
    # all_text：一个列表，包含训练集、验证集和测试集中所有的文本数据。
    vocab, all_text = load_data(train_data, valiate_data, test_data)
    ...
    #
    # 加载词向量模型
    word_embedding_path = "../Data/weibo1/word_embedding.pickle"
    #若不存在，新建词向量模型
    if not os.path.exists(word_embedding_path):
        ...
        for word in w2v.wv.index_to_key:  # 使用index_to_key获取词汇表
    ...
    # #
    print("word2vec loaded!")
    print("num words already in word2vec: " + str(len(w2v.wv.key_to_index)))
    # w2v = add_unknown_words(w2v, vocab)
    Whole_data = {}
    file_path = "../Data/weibo1/event_clustering.pickle"
    # if not os.path.exists(file_path):
    #     data = []
    #     for l in train_data["post_text"]:
    #         line_data = []
